<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="orderVenueName" label="场地名称">
      </el-table-column>
      ...
      <el-table-column prop="orderId" width="180" label="操作" fixed="right">
        <template slot-scope="scope">
          <el-popconfirm
            title="确定已经签到使用了吗？"
            @confirm="useOrder(scope.row.orderId)"
          >
            <el-button
              type="success"
              slot="reference"
              :disabled="
                !(hasAuth('venue:order:use') && scope.row.orderState == 1)
              "
              >使用</el-button
            >
          </el-popconfirm>
          ...
          <template>
            ...
            >
              <el-button
                ...
                :disabled="
                  !(
                    hasAuth('venue:order:cancel') &&
                    (scope.row.orderState == 0 || scope.row.orderState == 1)
                  )
                "
                >取消</el-button
              >
